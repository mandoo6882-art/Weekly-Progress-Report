...
export async function getTabs() {
  ...
  `;
  // tab_number가 같은 행(예: 7번 탭의 sub tab들)이 여러 개일 수 있어, DB의 임의 순서 대신
  // tabConfig.js에 정의된 순서(=업로드 카드/서브탭이 항상 같은 순서로 보이도록)로 재정렬.
  const order = new Map(TAB_CONFIGS.map((c, i) => [c.id, i]));
