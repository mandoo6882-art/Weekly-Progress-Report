...
export async function parseTabFile(tabId, arrayBuffer, filename) {
  ...
  if (tabId === 'equipment-control') {
    ...
    // S/T열("SPARE", 항상 0)은 화면에 보이지 않는 열이라 제거 (B=0 기준 idx17,18)
    grid = grid.map((row) => row.filter((_, idx) => idx !== 17 && idx !== 18));

    const dataColStart = 3; // 0,1,2열(Discipline/Category/Category)은 텍스트, 3열(Total)부터 숫자
    ...
    result.blocks.push({
      ...
      headerRowCount: 0,
      rowClasses,
      narrowCols: [19], // Remark열(마지막 열)은 폭을 좁게 고정하고 줄바꿈
