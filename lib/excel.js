...
// labelCol(예: 'G') 세로 방향으로 [Plan (IP/JP), Plan Inc. (IP/JP), Actual/Forecast,
// Actual/Forecast Inc., Variance] 5행이 연속으로 나오는 곳을 찾아, 그때마다 하나의 블록으로
// 인식해 차트/표를 만듭니다. 실제 시트 구조(확인됨):
//   - Plan 행 바로 위 행(planRow-1)에 "Total Qty/Completed/Balance/Plan vs Actual" 헤더와
//     날짜들이 같이 있음 → 날짜 행 = planRow-1
//   - Total/Completed/Balance 값은 planRow의 D/E/F열(5행에 걸쳐 세로 병합되어 있어 값은
//     맨 위 행에만 존재)
//   - 블록 제목(품목명)과 상위 구분(Overall/Train7/Train8/Train9)은 B열에서, planRow보다
//     위쪽에 떨어져 있는 두 줄(가까운 쪽=구분, 먼 쪽=품목명)로 존재
function extractLabelColumnBlocks(ws, {
  ...
  // "Plan (IP)"/"Plan (JP)" 등 IP/JP 표기 차이를 허용하는 라벨 매칭
  const LABEL_PATTERNS = [
    /^plan\s*\((?:i|j)p\)$/i,
    /^plan\s*inc\.?\s*\((?:i|j)p\)$/i,
    /^actual\s*\/\s*forecast$/i,
    /^actual\s*\/\s*forecast\s*inc\.?$/i,
    /^variance$/i,
  ];
  function matchesLabel(str, idx) {
    return LABEL_PATTERNS[idx].test(str.trim());
  }

  const blocks = [];
  let r = full.s.r;
  while (r <= full.e.r && blocks.length < maxBlocks) {
    const first = cellStr(r, labelColIdx);
    if (matchesLabel(first, 0)) {
      let matches = true;
      for (let i = 1; i < LABEL_PATTERNS.length; i++) {
        if (!matchesLabel(cellStr(r + i, labelColIdx), i)) {
          matches = false;
          break;
        }
      }
      if (matches) {
        ...
        // 데이터 열 범위: labelCol 다음 칸부터, 5개 행 중 값이 있는 가장 먼 열까지
        let lastDataCol = labelColIdx;
        [planRow, planIncRow, actualRow, actualIncRow, varianceRow].forEach((rr) => {
        ...
        if (lastDataCol > labelColIdx) {
          ...
          // 날짜 행 = Plan 행 바로 위 행(헤더 행과 같은 줄)
          const datesRow1based = planRow; // planRow는 0-based이므로 +0 하면 1-based로 planRow-1행이 됨

          // 블록 제목(품목명) / 상위 구분(Overall, Train7 등): planRow 위쪽으로 올라가며
          // B열에서 값이 있는 줄을 순서대로 최대 2개 수집. 가까운 쪽=구분, 먼 쪽=품목명.
          const foundAbove = [];
          for (let up = 1; up <= titleSearchUp && foundAbove.length < 2; up++) {
            const candidate = planRow - up;
            if (candidate < full.s.r) break;
            const v = cellStr(candidate, titleColIdx);
            if (v) foundAbove.push(v);
          }
          const category = foundAbove[0] || null; // Overall / Train7 / Train8 / Train9 등
          const title = foundAbove[1] || category || `Block ${blocks.length + 1}`;

          // D/E/F열: Total Qty / Completed / Balance (Plan 행에 값, 5행에 걸쳐 세로 병합됨)
          function qtyAt(colLetter) {
            const c = XLSX.utils.decode_col(colLetter);
            const v = cellRaw(planRow, c);
  ...
  if (blocks.length === 0) {
    ...
    throw new Error(
      `"${labelCol}"열에서 라벨(Plan / Plan Inc. / Actual/Forecast / Actual/Forecast Inc. / Variance)을 찾지 못했습니다.\n` +
