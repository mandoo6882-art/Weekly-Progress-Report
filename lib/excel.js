...
export async function parseTabFile(tabId, arrayBuffer, filename) {
  ...
  if (tabId === 'critical-milestone') {
    ...
    result.blocks.push({
      ...
      rows,
      headerRowCount: 2,
      colMaxWidths: { [lastColIdx]: 2080 }, // Remarks 열: 기존 폭(1040)의 2배
      scrollHeight: 600, // 표가 페이지를 무한히 늘리지 않도록 세로 스크롤
    });
  }
  ...
  if (tabId === 'procurement') {
    ...
    result.blocks.push({
      ...
      colMaxWidths: { [delayLastColIdx]: 700 }, // Delay Reason 열: 잘리지 않고 줄바꿈으로 다 보이도록
      filterColumn: 0, // Discipline열 기준 드롭다운 필터
      filterLabel: 'Discipline',
      scrollHeight: 600, // 세로 스크롤
    });
  }
