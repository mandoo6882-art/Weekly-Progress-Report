...
function extractRangeWithMerges(ws, rangeA1) {
...
export async function parseTabFile(tabId, arrayBuffer, filename) {
  ...
  if (tabId === 'overall-progress') {
    ...
    // E3:V11 고정 범위, 병합 셀 구조까지 그대로 유지. T열(E기준 15번째, 0-based)은 날짜,
    // H열(3번째)은 WT% 백분율.
    const progressGrid = extractRangeWithMerges(wsProgress, 'E3:V11');
    progressGrid.forEach((row) => {
      const tCell = row[15];
      if (tCell && isDateSerial(tCell.value)) tCell.value = excelSerialToDate(tCell.value);
    });
    const progressRows = removeBlankMergeRows(formatMergeGrid(progressGrid, [3]));
    result.blocks.push({
      type: 'table',
      title: 'Overall Progress (E3:V11)',
      rows: progressRows,
      headerRowCount: 3, // 제목행 + 헤더 2행
    });
