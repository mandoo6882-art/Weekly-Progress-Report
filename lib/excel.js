...
export async function parseTabFile(tabId, arrayBuffer, filename) {
  ...
  const wb = XLSX.read(arrayBuffer, { type: 'array', sheets: cfg.sheets });
  const asOf = guessAsOf(filename, cfg.filePrefix);
  const result = { sourceFile: filename, asOf, tables: [], charts: [] };
  ...
  if (tabId === 'overall-progress') {
    ...
    result.tables.push({
      title: 'Progress Table_weekly (JP) — 행 3~11',
      rows: extractRowBandTrimmed(wsProgress, 3, 11),
    });
    result.tables.push({
