...
// labelCol(예: 'G') 세로 방향으로 [Plan (JP), Plan (JP) Inc., Actual/FCST, Actual/FCST Inc., Variance (JP)]
// 5행이 연속으로 나오는 곳을 찾아, 그때마다 하나의 블록으로 인식해 차트1/차트2/표를 만듭니다.
// (WeeklyProgTrend처럼 고정 간격 반복이 아니라, 라벨 자체를 찾아서 위치를 판단합니다.)
function extractLabelColumnBlocks(ws, {
  labelCol = 'G',
  labels = ['Plan (JP)', 'Plan Inc. (JP)', 'Actual/Forecast', 'Actual/Forecast Inc.', 'Variance (JP)'],
  titleCols = ['B'],
  qtyCols = { total: 'D', completed: 'E', remain: 'F' },
  format = 'integer',
  maxDataCols = 60,
  maxBlocks = 60,
} = {}) {
  ...
  // 마침표/공백 차이에 영향받지 않도록 정규화(예: "Actual/Forecast Inc."와 "Actual/Forecast Inc" 동일 취급)
  ...
  const blocks = [];
  let r = full.s.r;
  while (r <= full.e.r && blocks.length < maxBlocks) {
    const first = normalize(cellStr(r, labelColIdx));
    if (first === normLabels[0]) {
      // 이어지는 4행이 나머지 라벨과 일치하는지 확인
      let matches = true;
      for (let i = 1; i < normLabels.length; i++) {
        if (normalize(cellStr(r + i, labelColIdx)) !== normLabels[i]) {
          matches = false;
          break;
        }
      }
      if (matches) {
        const planRow = r;
        const planIncRow = r + 1;
        const actualRow = r + 2;
        const actualIncRow = r + 3;
        const varianceRow = r + 4;

        // 데이터 열 범위: labelCol 다음 칸부터, Plan 행에서 값이 있는 만큼 자동으로 인식
        let lastDataCol = labelColIdx;
        for (let c = labelColIdx + 1; c <= Math.min(labelColIdx + maxDataCols, full.e.c); c++) {
          if (cellStr(planRow, c) !== '') lastDataCol = c;
        }
        // Actual/FCST 등 다른 행에도 값이 있을 수 있으니 4행 모두 확인해서 더 넓으면 갱신
        [planIncRow, actualRow, actualIncRow, varianceRow].forEach((rr) => {
          for (let c = labelColIdx + 1; c <= Math.min(labelColIdx + maxDataCols, full.e.c); c++) {
            if (cellStr(rr, c) !== '' && c > lastDataCol) lastDataCol = c;
          }
        });

        if (lastDataCol > labelColIdx) {
          const cols = [];
          for (let c = labelColIdx + 1; c <= lastDataCol; c++) cols.push(XLSX.utils.encode_col(c));

          // 날짜 행: 1) Plan 행 바로 위(최대 5행 위까지) 우선 탐색, 2) 없으면 시트 상단
          // (row1~30) 전체에서 날짜가 많이 발견되는 행을 공용 날짜 행으로 사용(모든
          // 블록이 같은 타임라인을 공유하는 구조일 수 있으므로).
          let datesRowIdx = null;
          for (let up = 1; up <= 5; up++) {
            const candidate = planRow - up;
            if (candidate < full.s.r) break;
            const hasDate = cols.some((cl) => {
              const c = XLSX.utils.decode_col(cl);
              const addr = XLSX.utils.encode_cell({ r: candidate, c });
              const cell = ws[addr];
              return cell && isDateSerial(cell.v);
            });
            if (hasDate) {
              datesRowIdx = candidate;
              break;
            }
          }
          if (datesRowIdx === null) {
            let bestRow = null;
            let bestCount = 0;
            for (let candidate = full.s.r; candidate <= Math.min(full.s.r + 30, full.e.r); candidate++) {
              const count = cols.reduce((acc, cl) => {
                const c = XLSX.utils.decode_col(cl);
                const addr = XLSX.utils.encode_cell({ r: candidate, c });
                const cell = ws[addr];
                return acc + (cell && isDateSerial(cell.v) ? 1 : 0);
              }, 0);
              if (count > bestCount) {
                bestCount = count;
                bestRow = candidate;
            ...
            if (bestCount >= 2) datesRowIdx = bestRow;
          }

          // 블록 제목: B열에서, Plan~Variance 5행 범위 안의 첫 값을 찾음(병합/특정 줄 무관)
          let title = '';
          let titleRow = planRow;
          for (let rr = planRow; rr <= varianceRow; rr++) {
            for (const tc of titleCols) {
              const c = XLSX.utils.decode_col(tc);
              const v = cellStr(rr, c);
              if (v) {
                title = v;
                titleRow = rr;
                break;
          ...
          if (!title) title = `Block ${blocks.length + 1}`;

          // D/E/F열: Total Qty / Completed Qty / Remain Qty (제목과 같은 행에 있는 값)
          function qtyAt(colLetter) {
            const c = XLSX.utils.decode_col(colLetter);
            const addr = XLSX.utils.encode_cell({ r: titleRow, c });
            const cell = ws[addr];
            const v = cell ? cell.v : '';
          ...
          const summary = {
            ...
            remainQty: qtyAt(qtyCols.remain),
          };

          const categoriesRow1based = (datesRowIdx !== null ? datesRowIdx : planRow - 1) + 1;
          const { chart1, chart2, table } = buildTrendChartsAndTable(ws, {
            datesRow: categoriesRow1based,
            planRow: planRow + 1,
            planIncRow: planIncRow + 1,
            actualRow: actualRow + 1,
            actualIncRow: actualIncRow + 1,
            varianceRow: varianceRow + 1,
            cols,
            labelCol,
            format,
          });

          blocks.push({ title, chart1, chart2, table, summary });
        }
  ...
  // 블록을 하나도 못 찾았다면 조용히 빈 화면을 보여주는 대신, 실제 라벨 열에서 발견된
  // 값들을 보여줘서 어떤 텍스트가 실제로 들어있는지 바로 알 수 있게 합니다.
  if (blocks.length === 0) {
    const samples = [];
    for (let rr = full.s.r; rr <= full.e.r && samples.length < 15; rr++) {
      const v = cellStr(rr, labelColIdx);
      if (v) samples.push(`행${rr + 1}: "${v}"`);
    }
    throw new Error(
      `"${labelCol}"열에서 라벨(${labels.join(' / ')})을 찾지 못했습니다.\n` +
      `실제 ${labelCol}열에서 발견된 값:\n${samples.join('\n') || '(값 없음)'}`
    );
  }
...
export async function parseTabFile(tabId, arrayBuffer, filename) {
  ...
  if (tabId === 'overall-progress') {
    ...
    result.blocks.push({ type: 'chart', title: 'EPC Overall Progress Trend (Plan/Actual)', categories: chart1.categories, series: chart1.series, format: chart1.format });
    result.blocks.push({ type: 'chart', title: 'EPC Overall Progress Trend (Variance)', categories: chart2.categories, series: chart2.series, format: chart2.format });
  ...
  if (tabId === 'key-qty') {
    ...
    // G열에 Plan (JP)/Plan (JP) Inc./Actual/FCST/Actual/FCST Inc./Variance (JP) 5행이
    // 연속으로 나오는 위치를 찾아 그때마다 블록으로 인식합니다(WeeklyProgTrend와 달리
    // 고정 43행 간격이 아니라 라벨 자체로 위치를 판단). 값은 정수로 표시합니다.
    const blocks = extractLabelColumnBlocks(wsQty, { labelCol: 'G', format: 'integer' });
    blocks.forEach((b) => {
      // tab4와 달리 Variance는 표에만 수치로 넣고, 별도 차트는 만들지 않습니다.
      result.blocks.push({
        type: 'chart',
        title: `Weekly QTY Curve — ${b.title}`,
        categories: b.chart1.categories,
        series: b.chart1.series,
        format: b.chart1.format,
      });
      // Total/Completed/Remain Qty를 날짜 헤더 행 바로 다음, 추이 수치 위쪽에 추가
      const tableWithSummary = [
        b.table[0],
        ['Total Qty', b.summary.totalQty],
        ['Completed Qty', b.summary.completedQty],
        ['Remain Qty', b.summary.remainQty],
        ...b.table.slice(1),
      ];
      result.blocks.push({
        type: 'table',
