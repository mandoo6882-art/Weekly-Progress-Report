...
function mergeCell(value, colSpan = 1) {
  return { value, colSpan, rowSpan: 1, hidden: false };
}
...
export async function parseTabFile(tabId, arrayBuffer, filename) {
  ...
  if (tabId === 'procurement') {
    ...
    result.blocks.push({
      ...
      // 내용이 넘치면 표 전체가 가로로 스크롤되도록 함(세로 스크롤 없음).
  ...
  if (tabId === 'schedule-cooling-tower') {
    ...
    const HEADERS = ['Activity', 'Duration', 'Plan Start', 'Plan Finish', 'Actual Start', 'Actual Finish', 'Remark'];
    const headerRow = HEADERS.map((h) => mergeCell(h));
    const bodyRows = items.map((it) => {
      if (it.isHeading) {
        const cells = HEADERS.map(() => ({ value: '', colSpan: 1, rowSpan: 1, hidden: true }));
        cells[0] = mergeCell(it.name, HEADERS.length);
        return cells;
      }
      return [
        mergeCell(it.name),
        mergeCell(it.duration ?? ''),
        mergeCell(isoToDisplay(it.planStart)),
        mergeCell(isoToDisplay(it.planFinish)),
        mergeCell(isoToDisplay(it.actualStart)),
        mergeCell(isoToDisplay(it.actualFinish)),
        mergeCell(it.remark),
      ];
    });
    const rowClasses = ['', ...items.map((it) => (it.isHeading ? 'cp-band-row' : ''))];
    ...
    result.blocks.push({
      ...
      headerRowCount: 1,
      rowClasses,
      colWidths: [28, 8, 12, 12, 12, 12, 16],
      colMaxWidths: { 6: 320 },
    });

    const ganttRows = items.map((it) =>
      it.isHeading
        ? { name: it.name, isHeading: true }
        : {
            name: it.name,
            isHeading: false,
            bars: [
    ...
    result.blocks.push({
      type: 'critical-path-gantt',
      title: `${cfg.name} — Monthly Schedule`,
      rows: ganttRows,
      legend: [
  ...
  if (tabId === 'schedule-ts003') {
    ...
    const HEADERS = [
      'Activity', 'Duration', 'Plan Start', 'Plan Finish',
      'Forecast Start', 'Forecast Finish', 'Actual Start', 'Actual Finish', 'Remark',
    ];
    const headerRow = HEADERS.map((h) => mergeCell(h));
    const bodyRows = items.map((it) => {
      if (it.isHeading) {
      ...
      return [
        ...
        mergeCell(isoToDisplay(it.planStart)),
        mergeCell(isoToDisplay(it.planFinish)),
        mergeCell(isoToDisplay(it.forecastStart)),
        mergeCell(isoToDisplay(it.forecastFinish)),
        mergeCell(isoToDisplay(it.actualStart)),
        mergeCell(isoToDisplay(it.actualFinish)),
        mergeCell(it.remark),
      ];
    ...
    result.blocks.push({
      ...
      headerRowCount: 1,
      rowClasses,
      colWidths: [22, 6, 9, 9, 9, 9, 9, 9, 18],
      colMaxWidths: { 8: 320 },
    });

    const ganttRows = items.map((it) =>
      it.isHeading
        ? { name: it.name, isHeading: true, depth: it.depth }
        : {
            name: it.name,
            isHeading: false,
            depth: it.depth,
