    ...
    if (first === normLabels[0]) {
      ...
      if (matches) {
        ...
        if (lastDataCol > labelColIdx) {
          ...
          // 날짜 행: Plan 행 바로 위(최대 5행 위까지)에서 날짜로 보이는 값이 있는 행을 찾음
          ...
          // 블록 제목: Plan 행 또는 그 바로 위 행의 titleCols 범위에서 첫 값을 찾음
          let title = '';
          let titleRow = planRow;
          for (const rowCandidate of [planRow, planRow - 1]) {
            if (rowCandidate < full.s.r) continue;
            for (const tc of titleCols) {
              const c = XLSX.utils.decode_col(tc);
              const v = cellStr(rowCandidate, c);
              if (v) {
                title = v;
                titleRow = rowCandidate;
                break;
              }
            }
            if (title) break;
