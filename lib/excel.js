...
export async function parseTabFile(tabId, arrayBuffer, filename) {
  ...
  if (tabId === 'key-qty') {
    ...
    // Construction 탭의 WeeklyProgTrend와 동일한 구조(43행 간격 반복 블록)를 그대로 사용하되,
    // 값은 백분율이 아닌 정수(수량)로 표시합니다.
    // ※ 실제 시트의 행 간격/오프셋이 WeeklyProgTrend와 다르면 조정이 필요할 수 있습니다.
    const blocks = extractWeeklyProgTrendBlocksV2(wsQty, { format: 'integer' });
    blocks.forEach((b) => {
      result.blocks.push({
        type: 'chart',
        title: `Weekly QTY Curve — ${b.title} (Plan/Actual)`,
        categories: b.chart1.categories,
        series: b.chart1.series,
        format: b.chart1.format,
      });
      result.blocks.push({
        type: 'chart',
        title: `Weekly QTY Curve — ${b.title} (Variance)`,
        categories: b.chart2.categories,
        series: b.chart2.series,
        format: b.chart2.format,
      });
