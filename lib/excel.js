...
function extractLabelColumnBlocks(ws, {
  labelCol = 'G',
  labels = ['Plan (JP)', 'Plan (JP) Inc.', 'Actual/FCST', 'Actual/FCST Inc.', 'Variance (JP)'],
  titleCols = ['B'],
  ...
  while (r <= full.e.r && blocks.length < maxBlocks) {
    const first = cellStr(r, labelColIdx).toLowerCase();
    if (first === normLabels[0]) {
      // 이어지는 4행이 나머지 라벨과 일치하는지 확인
      let matches = true;
      for (let i = 1; i < normLabels.length; i++) {
        if (cellStr(r + i, labelColIdx).toLowerCase() !== normLabels[i]) {
          matches = false;
          break;
        }
      }
